#!/bin/bash
# The body below the "SHARED PIPELINE" marker must be byte-identical across the
# three sweep scripts. Run from anywhere; exits non-zero on drift.
set -u
cd "$(dirname "$0")"
...
ok=1
for f in sweep_SAML.py sweep_IBM.py; do
